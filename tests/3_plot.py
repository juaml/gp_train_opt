...
for fname in results_path.glob(
    f"results_{data_kind}_*_splits_*_features_cv_scores.csv"
):
    t_df = pd.read_csv(fname)
    n_splits = int(fname.stem.replace(f"results_{data_kind}", "").split("_")[1])
    if n_splits == 0:
...
axes[0].legend(loc=1, prop={"size": 8}, title="# Features")
axes[0].set_xlabel("# Splits")
axes[0].set_ylabel("Fit time (s)")
axes[0].set_xticklabels(["No Split", "2", "3", "4", "5"])
sns.swarmplot(
    x="n_splits",
    y="test_neg_mean_absolute_error",
    data=stats_df,
...
axes[1].set_xticklabels(["No split", "2", "3", "4", "5"])
fig.subplots_adjust(wspace=0.2)

fig.suptitle(f"fastGPR performance by number of features and splits")
plt.savefig(
    figs_path / f"fig_{data_kind}_features_splits.pdf",
    bbox_inches="tight")
...
dfs = []
for fname in results_path.glob(f"results_{data_kind}_*_splits_*_repeats_cv_scores.csv"):
    t_df = pd.read_csv(fname)
    n_splits = int(fname.stem.replace(f"results_{data_kind}", "").split("_")[1])
    if n_splits == 0:
        n_splits += 1
...
axes[0].legend(loc=1, prop={"size": 7}, title="# Repeats")
axes[0].set_xlabel("# Splits")
axes[0].set_ylabel("Fit time (s)")
sns.swarmplot(
    x="n_splits",
...
fig.suptitle(f"fastGPR performance by number of splits and repeats")
plt.savefig(
    figs_path / f"fig_{data_kind}_repeats_splits.pdf",
    bbox_inches="tight")
